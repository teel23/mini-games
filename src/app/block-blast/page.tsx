...
export default function BlockBlastPage() {
  ...
  const ghostCells = hoverCell
    ? new Set(game.getGhost(hoverCell[0], hoverCell[1]).map(([r, c]) => `${r}-${c}`))
    : new Set<string>();

  const handleCellClick = useCallback((r: number, c: number) => {
    if (game.selectedPiece !== null) {
      const ghost = game.getGhost(r, c);
      if (ghost.length > 0) {
        haptic.light();
        playTick();
      } else {
  ...
  // Touch: track finger over grid to show ghost preview
  const handleGridTouchMove = useCallback((e: React.TouchEvent) => {
    if (game.selectedPiece === null || !gridRef.current) return;
    e.preventDefault();
      ...
      <div className="flex justify-center px-2 overflow-x-auto">
        <div
          ...
          style={{
            ...
            borderRadius: 12,
            padding: 8,
            touchAction: game.selectedPiece !== null ? 'none' : 'auto',
          }}
        >
              ...
              return (
                <div
                  ...
                  style={{
                    width: CELL_PX,
                    height: CELL_PX,
                    background: cellVal ?? (isGhost
                      ? (game.pieces[game.selectedPiece!]?.color ?? '#888') + '55'
                      : '#242424'),
                    border: isGhost
                      ? `2px solid ${game.pieces[game.selectedPiece!]?.color ?? '#888'}`
                      : '1px solid #2e2e2e',
                    borderRadius: 4,
      ...
      {game.selectedPiece !== null && (
        <p className="text-center text-xs mt-2" style={{ color: '#888' }}>
          {typeof window !== 'undefined' && 'ontouchstart' in window
            ? 'Drag finger over grid · tap to place'
